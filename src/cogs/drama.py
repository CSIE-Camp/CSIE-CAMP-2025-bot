...
class Drama(commands.Cog):
    ...
    @app_commands.command(name="startedrama")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.default_permissions(
        discord.Permissions(administrator=True), manage_messages=True
    )
    async def startedrama(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="抽籤結果", description="以下是報告的順序", color=0xFF0000
        )
        for i, name in enumerate(names):
            embed.add_field(name=f"第 {i + 1} 組報告", value=name, inline=False)

        await starfish_webhook.send(
            "呼，我到了", username=starfish_name, avatar_url=starfish_avatar
        )
        await asyncio.sleep(3)
        await starfish_webhook.send(
            "等我一下喔，現在要抽籤對吧",
            username=starfish_name,
            avatar_url=starfish_avatar,
        )
        await asyncio.sleep(3)
        await starfish_webhook.send(
            "ㄟ！大傢伙，幫我抽個籤", username=starfish_name, avatar_url=starfish_avatar
        )
        ...
        msg = await starfish_webhook.send(
            "好啦好啦，會給會給啦，快做事啦",
            username=starfish_name,
            avatar_url=starfish_avatar,
            wait=True,
