...
class Notes(commands.Cog):
    ...
    @app_commands.describe(
        show_publicly="。如果選False，則僅你可見；如果選True，則頻道中其他人皆可見。"
    )
    async def list_notes(
        self, interaction: discord.Interaction, show_publicly: bool = False
    ):
        note_names = list(self.notes_json.keys())
        if len(note_names) == 0:
            await interaction.response.send_message(
                "目前沒有任何筆記。", ephemeral=not show_publicly
            )
            return
        notes_list = "\n".join(
            f"- `{name}` — {self.notes_json[name]['ch_comment']}" for name in note_names
        )
        embed = discord.Embed(
            title="筆記一覽", description=notes_list, color=discord.Color.blue()
        )
        await interaction.response.send_message(
            embed=embed, ephemeral=not show_publicly
        )
    ...
    async def search(
        ...
        status_message = await interaction.followup.send(
            f"沒有找到名為「{keyword}」的筆記……", ephemeral=not show_publicly
        )
        ...
        async with typing_context.typing():
            ...
            closest_name_response = await llm_model.generate_content_async(prompt)
            closest_name = closest_name_response.text.strip()

            if not self.notes_json.get(closest_name):
                interaction.followup.send(
                    "抱歉，現在頭腦有點燒機了…", ephemeral=not show_publicly
                )
                print(
                    f"ERROR(src.cogs.notes): searching {closest_name} from {keyword} by LLM, but it's not in note list."
                )
                return
